...
export default class RenderCategory extends Component {
     ...
     */
    selectedData = (id, senderName, categories, selectedCatId, snoozeValue, timerValue) => {
        console.log('test cat', id, senderName, categories, selectedCatId);
        this.props.setSelected(id, senderName, categories, selectedCatId, snoozeValue, timerValue);
    };
    ...
    render() {
        ...
        return (
            <View style={innerStyle.gridViewContainer} key={index}>
                <TouchableOpacity onLongPress={() => this.selectedData(item.sender_id, item.sender_name, item.category_id, item.sender_category_id, item.is_snooze, item.timer_id)} >
                    <View style={{ flexDirection: 'row' }}>
                        <View style={innerStyle.imageView}>
                            {item.unread_count !== '00' && <View style={{ position: 'absolute', zIndex: 999, left: 27, bottom: 28 }}>
                            ...
                            {this.props.onCheckData && this.props.isSelected === item.sender_id ?
                                <TouchableOpacity onPress={() => this.handleSelectedIcon()}>
                                ...
                                :
                                item.is_snooze ? <View style={innerStyle.snoozeInnerStyle}>
                                    <Image
                        ...
                        <TouchableOpacity onLongPress={() => this.selectedData(item.sender_id, item.sender_name, item.category_id, item.sender_category_id, item.is_snooze, item.timer_id)} onPress={() => this.handleSelection(item.sender_name)}>
                            <View style={innerStyle.outerView}>
                             <View>
                                    <Text style={innerStyle.categoryText}>
                                    {item.sender_name}</Text>
                                    <Text style={innerStyle.emailTextEmailId} numberOfLines={2}
                                     ellipsizeMode="tail">
                                        {'(' + item.sender_email + ')'}
                                ...
                                <View style={innerStyle.titleView}>
                                    ...
                                    <Text style={[innerStyle.emailText, { marginTop: 6, fontSize: 0.038 * GLOBLE.DEVICE_WIDTH, color: '#034CBB', marginLeft: 8 }]}>
                                        {item.category_name}
                                    </Text>
                                </View>
                            </View>
...
const innerStyle = StyleSheet.create({
    ...
    categoryText: {
        fontSize: 0.036 * GLOBLE.DEVICE_WIDTH,
