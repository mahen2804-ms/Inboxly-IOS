...
import { EDIT_CATEGORY_SAGA_REQUEST, EDIT_CATEGORY_SUCCESS, SENDER_LIST_FAILURE, SENDER_LIST_REQUEST, SENDER_LIST_SAGA_REQUEST, SENDER_LIST_SUCCESS, SNOOZ_SENDER_SAGA_REQUEST, SNOOZ_SENDER_SUCCESS, SNOOZE_LIST_SAGA_REQUEST, SNOOZ_LIST_SUCCESS, UNSNOOZE_SENDER_SUCCESS, UNSNOOZE_SAGA_REQUEST } from '../redux/actionType';
...
function* senderListRequestSaga(action) {
    ...
    try {
        yield put({ type: SENDER_LIST_REQUEST });
        const senderList = yield call(
            fetchApi,
            ...[`${API.senderList}`],
        );
        console.log('sender list response', senderList);
        yield put({ type: SENDER_LIST_SUCCESS, payload: senderList && senderList.data && senderList.data.success && senderList.data.success.data });
