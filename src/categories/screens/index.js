...
import { MainHeader, SwipableListItem, ModalComponent, Loader, NoContentFound } from '../../components';
import { Fonts } from '../../utils/Fonts';
import { createCategoryAction, categoryListAction, updateCategoryAction, categoryStatusAction } from '../action';
        ...
        this.setState({ showModal: false }, () => {
            if (categoryId == '') {
                ...
                this.props.updateCategoryAction(requestData, res => {
                    if (res.status === STATUS_CODES.OK) {
                        Toast.showToast(TOASTER_LABEL.UPDATE_CATEGORY, 'success');
                        this.props.categoryListAction(res => {
                            if (res.status === STATUS_CODES.OK) {
                                this.setState({ refresh: true, searchText: '', categoryId: '' });
                            }
                        });
                    }
                });
    ...
    renderListSection = () => {
        return (
            <FlatList
                ...
                data={this.props.categoryData && this.props.categoryData.length > 0 ? this.props.categoryData : this.state.listItem}
    ...
    render() {
        ...
        return (
            <Container>
                ...
                <MainHeader leftButtonType={'menu'} rightButtonType={'add-circle-outline'} leftButton={true} rightButton={true} title={LABELS.MANAGE_CATEGORY} onPress={() => this.setState({ showModal: true })} />
                <Content>
...
export default connect(mapStateToProps, { createCategoryAction, categoryListAction, updateCategoryAction, categoryStatusAction })(Categories);