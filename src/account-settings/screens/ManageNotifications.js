...
function ManageNotifications(props) {
  ...
  const ListData = () => {
    return listDatalis.map((element, i) => {
      return        <View style={{ flexDirection: "row", height: 55, }}>
        <Text style={innerStyle.textStyle}>{element.title}</Text>
        <Switch
          trackColor={{ false: "grey", true: "#034CBB" }}
          thumbColor={element.status == 1 ? "grey" : "#034CBB"}
          ios_backgroundColor="gray"
          onValueChange={(value) => updateNotificationListPreference(element.id, value,i)}
          value={element.status == 1 ? true : false}
          style={{
            transform:
              Platform.OS == "ios"
  ...
  //     thumbColor={element.status == 1 ? "grey" : "#034CBB"}
  //     ios_backgroundColor="gray"
  //     onValueChange={(value) => updateNotificationListPreference(element.id, value)}
  //     value={element.status == 1 ? true : false}
  //     style={{
  //       transform:
  //         Platform.OS == "ios"
  ...
  const getListofItems = (value) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios.get('https://app.myinboxly.com/api/v1/user-notification-preference-list', {
        ...
        // ListData(response.data.success.data)
  ...
  const updateNotificationListPreference = (val, val1,index) => {
    // setIsLoading(true);
    let requestData = {
      id: val,
      status: val1,
     ...
     new Promise((resolve, reject) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          // let index = listDatalis.fi(val)
          listDatalis[index].status = listDatalis[index].status ? false : true
          setlistDeatils([...listDatalis])
          return resolve(response);
