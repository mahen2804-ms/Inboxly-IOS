...
class Helpers {
     ...
     */
    public static function sendEmail($template, $data, $toEmail, $toName, $subject, $fromName = '', $fromEmail = '',$attachment = '', $mime =  '') {

        \Mail::send($template, $data, function ($message) use($toEmail, $toName, $subject, $data, $fromName, $fromEmail, $attachment , $mime) {
    ...
    public static function uploadDocuments($document, $dir, $fileName = '') {
        ...
        if ($fileName == '') {
            $documentOriginalName = $currentTimeStamp .'.' . $document->getClientOriginalExtension();
        } else {
            $documentOriginalName = $currentTimeStamp.$fileName;
     ...
     * @return string
     */
    public static function uploadDocument($document, $dir, $fileName = '') {
        $date = new DateTime();
        $currentTimeStamp = $date->getTimestamp();
    ...
    public static function notificationAndroid($notificationType, $notifcationMessage, $notifcationInfo, $deviceToken, $additionalData, $title='') {
        //$serverKey = config('constant.FCM_LEGACY_SERVER_KEY');
        //$serverKey = 'AIzaSyAhM2Mf7ADn81zkqJVguTswgmvdpo7_hgM';
        $serverKey = 'AIzaSyBXAOwKOZJFpnWuYcoG8iBV51kv-v3e45A';
        ...
        if(!empty($additionalData)) {
            $fields = array(
                ...
                'notification' => array('body' => $notifcationMessage, 'title' => $title),
                'data' => array('message' => $notifcationMessage, 'sound' => 'default', 'senderName' => $additionalData['senderName'], 'companyName' => $additionalData['companyName'], 'title' => $title, 'notification_type' => $notificationType)
            );
        } else {
            $fields = array(
                'registration_ids' => $GcmRegistrationId,
                'notification' => array('body' => $notifcationMessage, 'title' => $title),
                'data' => array('message' => $notifcationMessage, 'sound' => 'default', 'title' => $title, 'notification_type' => $notificationType)
            );
        }
    ...
    */
     ...
     * @return bool
     */
    public static function set_sql_mode($mode = '')
    {
        return \DB::statement("SET SESSION SQL_MODE=''");
    }
     ...
     * @return array
     */
    public static function checkDateDifference($quesDate, $draftDate) {

        $qDate = new DateTime($quesDate);
        $dDate = new DateTime($draftDate);

        if ($qDate > $dDate) {
            //delete draft of login user
            $deleteDraft = AssessmentDraft::deleteDraft(Auth::user()->id);
        }
    ...
    * @desc Record is exist in db 
    * @return array
    */
    public static function checkDeletedRecord($id, $tableName) {
        try {
            
            $record = $tableName::where('id', $id)->count();
    ...
    /**
    * @name checkAlreadyChangedStatus
    * @desc Record is exist in db 
    * @return array
    */
    public static function checkAlreadyChangedStatus($id, $tableName, $status) {
        try {
            $statusRecord = $tableName::findOrFail($id);

            if(!empty($statusRecord)  && $status == $statusRecord->status) {
                return 1;
            } else {
                return 0;
            }
